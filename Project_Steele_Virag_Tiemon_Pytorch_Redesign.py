# Project_Steele_Virag_Tiemon_Pytorch_Redesign.py - Eric Steele, Mate Virag, Liam Tiemon
# NKU CSC/DSC 494/594 Deep Learning Spring 2018 K. Kirby
# ---------------------------------------------------------------------
# The Project_Steele_Virag_Tiemon_Pytorch_Redesign.py file creates and trains a 
# neural network using the PyTorch machine learning library to solve the
# Myanmar/Devanagari classification problem. The neural network used
# in this  This file requires NkuMyaDevMaker.py to generate the images.
...
class Net(nn.Module):
    ...
    def forward(self, x, pooling):
        ...
        # Activation functions are specified in this function even for the convolutional layer
...
"""
Dataset class for use with PyTorch's DataLoader. Takes in lists of images and
...
def main():
    ...
    # Declare hyperparameters, tuples define hyperparameters for multiple layers
    convo_kernels = (8,10)
    convo_kernel_size = 5
    convo_padding = 0
    pooling = 2
    hidden_neurons = (120, 84)
    batch_size = 100
    num_epochs = 8
    learning_rate = 0.001
    ...
    # Result of convolving nxn image twice (stride 1, with no padding) and square pooling 
    # will be n_conv x n_conv
    n_conv = ((n-(convo_kernel_size-1)) // pooling) - (convo_kernel_size-1)
    ...
    # Loss and Optimizer
    # Using mean squared error for our loss and Adam for our optimimzer
